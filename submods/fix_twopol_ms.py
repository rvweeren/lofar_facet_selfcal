...
"""
Reinout van Weeren
based on work by Martijn Oei and Francesco de Gasperin

Adapt the 2-pol MS format of data to one usable by LOFAR software.
...
class MS(object):
    ...
    def fix_weights(self):
        '''
        Fix WEIGHT column, put them at values of XX/RR and YY/LL.
        '''
        logging.info("- Adaptation of WEIGHT column -")
        
        logging.info("Fixing weights in MS %s", self.ms_file)

        if not self.columnExists("WEIGHT_SPECTRUM"):
            logging.warning("WEIGHT_SPECTRUM column not found, skipping.")
            return

        # Check shape using a single-row sample
        sample = self.t.getcol("WEIGHT_SPECTRUM", startrow=0, nrow=1)
        if sample.shape[-1] != 4:
            logging.warning("Expected 4 pols in WEIGHT_SPECTRUM, found %d. Skipping.", sample.shape[-1])
            return

        total_rows = self.t.nrows()
        for start_row in range(0, total_rows, self.chunk_size):
            nrow = min(self.chunk_size, total_rows - start_row)
            chunk = self.t.getcol("WEIGHT_SPECTRUM", startrow=start_row, nrow=nrow)
            # chunk shape: (nrow, nchan, 4)
            chunk[:, :, 1] = chunk[:, :, 0]  # XY/RL = XX/RR
            chunk[:, :, 2] = chunk[:, :, 3]  # YX/LR = YY/LL
            self.t.putcol("WEIGHT_SPECTRUM", chunk, startrow=start_row, nrow=nrow)
            del chunk
            gc.collect()
    ...
    def updatePolarisation(self):
        ...
        if np.size(corr_type) == 4:
            logging.info("Polarisation metadata already correct.")
            return

        if np.array_equal(np.array([[9, 12]]), corr_type): # linear polarisation
            correlationTypesNew      = np.array([[9, 10, 11, 12]])  # XX, XY, YX, YY
        elif np.array_equal(np.array([[5, 8]]), corr_type): # circular polarisation
            correlationTypesNew      = np.array([[5, 6, 7, 8]])      # RR, RL, LR, LL
        else:
            logging.error("Unknown CORR_TYPE format:")
            logging.error(corr_type)
            raise ValueError("Cannot update polarisation metadata due to unknown CORR_TYPE format.")
...
def readArguments():
    import argparse
    parser = argparse.ArgumentParser("Adapt two-pol MS to LOFAR MS format.")
